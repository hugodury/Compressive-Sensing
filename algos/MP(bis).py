...
def MP(x, D, kmax, epsilon):
    ...
    while k < kmax and np.linalg.norm(residuel) > epsilon:
        ...
        P.append(mk)
        zmk = (D[:,mk].T @ residuel) / np.linalg.norm(D[:,mk])**2
        
        alpha[mk] = alpha[mk] + zmk
        residuel = residuel - zmk * D[:, mk]
